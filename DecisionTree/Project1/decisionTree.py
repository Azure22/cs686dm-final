...
class DecisionTree():
    ...
    def entropy(self, labels):
        ...
        '''
        result = 1.0;
        targetLength = len(labels);
        targetList = {};
        for i in range(targetLength):
            if targetList.has_key(labels[i]):
                targetList[labels[i]] += 1;
            else:
                targetList[labels[i]] = 1;
        for i in range(len(targetList)):
            temp = float(targetList.values()[i])/float(targetLength);
            result = result *(- temp*math.log(temp,2));
        return result;
    ...
    def remainder(self, data) :
        ...
        for value in possibleValues :
            ...
            r += (float(c) / len(data) ) * self.entropy([item[1] for item in data if item[0] == value]);
        return r;
